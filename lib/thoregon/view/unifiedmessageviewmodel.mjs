...
import { ViewModel, AuroraView }  from "/thoregon.aurora"
...
export default class UnifiedMessageViewmodel extends ViewModel {
    ...
    initModelMapping() {
        if (this.east && this.east instanceof AuroraView) return;       // todo [REFACTOR]
...
/*
    async connectReplies() {
        ...
        query.addCurrentItemsHandler((items) => this.initialReplies(items));
        query.connect();
    }
*/
    ...
    async connectReactions() {
        ...
        if (!Query) {
            console.log(`AuroraCollection: Query '${reactionsQuery}' not found.`);
            return;
        }
        let query = Query.withParent(parent);
        this.query = query;
...
//            let channel = reply.channel
                ...
                if (me.alias === unifiedMessage.sender) elem.setAttribute('sent', '');
...
*/
    ...
    from() {
        ...
        return me || (guest ? JSON.parse(guest) : undefined) ;
    }
